...
# Estilização visual romântica corrigida para novas versões do Python
st.html("""
    <style>
...
st.write("<p style='text-align: center; font-size: 16px; color: #555;'>Um cantinho especial para lembrar de cada pedacinho da nossa caminhada juntos...</p>", unsafe_with_html=True)
st.write("---")
...
st.write("<p style='text-align: center; color: #666; font-size: 14px;'>Clique nos momentos abaixo para abrir cada lembrança...</p>", unsafe_with_html=True)
...
def criar_momento(numero, titulo, texto, nome_foto):
    with st.expander(f"✨ Momento {numero}: {titulo}"):
        st.write(f"<p style='font-size: 15px; color: #333; margin-bottom: 15px;'>{texto}</p>", unsafe_with_html=True)
        try:
...
# --- SEUS 12 MOMENTOS ---
# Você pode alterar as frases entre as aspas para colocar suas próprias memórias!
...
if st.button("Clique para abrir a surpresa final"):
    ...
    st.success("Eu te amo daqui até a Lua! 🌙Eu, vc, Nino e pix SEMRE! Obrigado por esses 12 momentos e por todos os outros que ainda vamos viver. Nosso futuro vai ser lindo! ❤️")